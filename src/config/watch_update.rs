    ...
    fn apply_general_watcher_key(&mut self, update: &Self, key: &str) -> bool {
        match key {
            ...
            "ollama_cloud_enablement" => {
                self.ollama_cloud_enablement = update.ollama_cloud_enablement;
            }
    ...
    fn apply_account_watcher_key(&mut self, update: &Self, key: &str) {
        match key {
            ...
            "selected_ollama_cloud_account_ids" => {
                self.selected_ollama_cloud_account_ids =
                    update.selected_ollama_cloud_account_ids.clone();
            }
            "ollama_cloud_managed_accounts" => {
                self.ollama_cloud_managed_accounts = update.ollama_cloud_managed_accounts.clone();
            }
            _ => {}
        }
    }
