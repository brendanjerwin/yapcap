...
fn reauth_capability_satisfied(
    ...
    match provider {
        ...
        | ProviderId::Antigravity
        | ProviderId::OpencodeGo
        | ProviderId::OllamaCloud => action_support.is_some_and(|support| support.can_reauthenticate),
    }
}
...
fn reauth_tooltip(provider: ProviderId) -> String {
    match provider {
        ...
        ProviderId::OllamaCloud => fl!("ollama-cloud-account-reauth-tooltip"),
    }
}
...
fn opencode_go_account_requires_action(account: &ProviderAccountRuntimeState) -> bool {
    account.provider == ProviderId::OpencodeGo && account.auth_state == AuthState::ActionRequired
}

fn ollama_cloud_account_requires_action(account: &ProviderAccountRuntimeState) -> bool {
    account.provider == ProviderId::OllamaCloud && account.auth_state == AuthState::ActionRequired
}

pub(super) fn opencode_go_account_settings_row<'a>(
    account: &'a ProviderAccountRuntimeState,
    selected_ids: &[&str],
    active_id: Option<&str>,
    config: &Config,
    ...
    let is_selected = selected_ids.contains(&account.account_id.as_str());
    let is_active = active_id == Some(account.account_id.as_str());
    let requires_action = opencode_go_account_requires_action(account);
    let account_id = account.account_id.clone();
    ...
    let can_delete = account_action_support(config, ProviderId::OpencodeGo, account_id.as_str())
        .is_some_and(|support| support.can_delete);
    let delete_press =
        (enabled && can_delete).then_some(Message::DeleteOpencodeGoAccount(account_id.clone()));
    let mut actions = row![account_selected_marker(is_selected, enabled)]
        .spacing(0)
        .align_y(Alignment::Center);
    if enabled && requires_action {
        actions = actions.push(account_action_icon_button(
            "view-refresh-symbolic",
            fl!("opencode-go-account-reauth-tooltip"),
            Some(Message::ReauthenticateOpencodeGoAccount(account_id)),
        ));
    }
    actions = actions.push(account_action_icon_button(
        "edit-delete-symbolic",
...
pub(super) fn ollama_cloud_account_settings_row<'a>(
    account: &'a ProviderAccountRuntimeState,
    selected_ids: &[&str],
    active_id: Option<&str>,
    config: &Config,
    ...
    let is_selected = selected_ids.contains(&account.account_id.as_str());
    let is_active = active_id == Some(account.account_id.as_str());
    let requires_action = ollama_cloud_account_requires_action(account);
    let account_id = account.account_id.clone();
    let mut title_row = row![account_label_text(&account.label, 14)]
        .spacing(8)
        .align_y(Alignment::Center)
    ...
    let can_delete = account_action_support(config, ProviderId::OllamaCloud, account_id.as_str())
        .is_some_and(|support| support.can_delete);
    let delete_press =
        (enabled && can_delete).then_some(Message::DeleteOllamaCloudAccount(account_id.clone()));
    let mut actions = row![account_selected_marker(is_selected, enabled)]
        .spacing(0)
        .align_y(Alignment::Center);
    if enabled && requires_action {
        actions = actions.push(account_action_icon_button(
            "view-refresh-symbolic",
            fl!("ollama-cloud-account-reauth-tooltip"),
            Some(Message::ReauthenticateOllamaCloudAccount(account_id)),
        ));
    }
    actions = actions.push(account_action_icon_button(
        "edit-delete-symbolic",
