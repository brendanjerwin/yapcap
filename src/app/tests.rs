...
fn opencode_go_account(id: &str) -> crate::config::ManagedOpencodeGoAccountConfig {
    crate::config::ManagedOpencodeGoAccountConfig {
        ...
        auth_cookie_source: "env:OPENCODE_GO_AUTH_COOKIE".to_string(),
        created_at: Utc::now(),
        updated_at: Utc::now(),
        last_authenticated_at: None,
    }
...
fn ollama_cloud_account(id: &str) -> crate::config::ManagedOllamaCloudAccountConfig {
    crate::config::ManagedOllamaCloudAccountConfig {
        id: id.to_string(),
        label: id.to_string(),
        session_cookie_source: "env:OLLAMA_CLOUD_SESSION_COOKIE".to_string(),
        created_at: Utc::now(),
        updated_at: Utc::now(),
        last_authenticated_at: None,
    }
        ...
        match provider {
            ...
            ProviderId::OllamaCloud => {
                app.config
                    ...
                    .push(ollama_cloud_account(keep_id));
                app.config
                    .ollama_cloud_managed_accounts
                    .push(ollama_cloud_account("remove"));
                app.config.selected_ollama_cloud_account_ids = vec![keep_id.to_string()];
                remove_account_id = "remove".to_string();
            }
        }
