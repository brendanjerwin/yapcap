...
pub(super) fn start_login(app: &mut AppModel, provider: ProviderId) -> Task<Message> {
    match provider {
        ...
        ProviderId::OllamaCloud => login::start_login::<login::OllamaCloudLoginFlow>(app),
...
pub(super) fn cancel_login(app: &mut AppModel, provider: ProviderId) {
    match provider {
        ...
        ProviderId::OllamaCloud => login::cancel_login::<login::OllamaCloudLoginFlow>(app),
        ProviderId::Cursor => {}
    }
}
...
pub(super) fn reauthenticate(
    ...
    match provider {
        ...
        ProviderId::OllamaCloud => {
            login::reauthenticate::<login::OllamaCloudLoginFlow>(app, account_id)
        }
